				...
				members : {
					...
					onAddChildWidget: function(e)
					{
						...
						if(optionsPanel.getChildren()[1].classname == "webfrontend.ui.QuickUseButton")
						{
                            ...
                            widget.add(child,{top: 3, left:165});
						}
					},
					tweakPA : function() {
						...
						try {
	                        ...
	                        this.app.getForumPostPage().getChildren()[0].add(btnRow, {top:45, left:400});

	                        // Add Scroll To Top button
	                        var scrollTBtn = new qx.ui.form.Button('Scroll To Top');
	                        scrollTBtn.set({width: 90, appearance: "button-text-small", toolTipText: "Scroll to top of thread"});
						...
						try
						{
							...
							var row = new qx.ui.container.Composite();row.setLayout(new qx.ui.layout.HBox(2));
							var btn = new qx.ui.form.Button("Inactives");
							row.add(btn);
							btn.setToolTipText("Ask MercBot to find members with no updates in the last 3 days");
                            btn.addListener("click", function() {webfrontend.data.Chat.getInstance().addMsg("/whisper MercBot !ai," + aip.getAlliance() + ",3");}, false);
							...
							var row = new qx.ui.container.Composite();row.setLayout(new qx.ui.layout.HBox(2));
							var btn = new qx.ui.form.Button("Player History");
							row.add(btn);
							btn.setToolTipText("Get player history from MercBot");
                            btn.addListener("click", function() {webfrontend.data.Chat.getInstance().addMsg("/whisper MercBot !history," + pip.getData().sName);}, false);
                        ...
                        var srb = gsc(4);
                        if (srb != null) {
                            srb.addListener("click", function() {usub('R')}, false);
                        }
                        else
                            paDebug('srb is null');
						window.setTimeout(usub, 1000);
                    ...
                    calcReturnTimes:function() {
                        ...
                        for(var i=0;i<orders.length;i++){
                            ...
                            if (order.type == SUPPORT_ORDER_ID) continue; // support won't return immediately
                            if (order.type == SIEGE_ORDER_ID) continue; // siege won't return immediately
					...
					createRaidApplyToAll : function() {
						try
						{
							...
							orderPage.applyAllBtn.addListener("execute", function(e) {
								var currRecurrType = orderPage.__btK.getRaidMode();
								var endStep = orderPage.__btK.getStepTime();
							...
							orderPage.applyAllThisDungeonBtn.addListener("execute", function(e) {
								...
								var city = orderPage.__cz.command.city; // FIXED
								var orders = webfrontend.data.City.getInstance().unitOrders;
								//var endStep = orderPage.calculateTimeInputStep();
								for (var i in orders) {
							...
							// BROKEN orderPage.__bbh.$$parent.add(btnRow, orderPage.__btK);
						}
						catch (e) {paDebug("apply options buttons error: " + e)}
					},
			...
			qx.Class.define("paTweak.Command", {
				...
				members : {
					...
					id : function(data, sender) {
						if (sender == 'MercWeb') {
							if ( typeof UReportTools.ViewData != 'undefined') {
								var vd = UReportTools.ViewData.getInstance();
								vd.getCityInfo(data);
							}
						}
						return false;
					...
					cm : function(data, sender) {
						if (sender == 'MercWeb') {
							paTweak.Chat.getInstance().addChatMessage(i18n('ext:merc_message') + ': ' + data.charAt(0).toUpperCase() + data.slice(1));
						}
						return false;
					},
					ok : function(data, sender) {
						paTweak.Chat.getInstance().addChatMessage(i18n('ext:ok_message') + ': ' + data.charAt(0).toUpperCase() + data.slice(1) + '@' + sender, true);
						return false;
					},
					like : function(data, sender) {
						paTweak.Chat.getInstance().addChatMessage('[<span id="CHAT_SENDER_' + sender + '" style="cursor:pointer;">' + sender + '</span>]' + i18n('ext:like_message'));
						return true;
					},
					poke : function(data, sender) {
						paTweak.Chat.getInstance().addChatMessage('[<span id="CHAT_SENDER_' + sender + '" style="cursor:pointer;">' + sender + '</span>]' + i18n('ext:poke_message'));
						return true;
					},
					vote : function(data, sender) {
						paTweak.Chat.getInstance().addChatMessage('[<span id="CHAT_SENDER_' + sender + '" style="cursor:pointer;">' + sender + '</span>]' + i18n('ext:vote_message'));
						return true;
					},
					love : function(data, sender) {
						paTweak.Chat.getInstance().addChatMessage(i18n('ext:love_message'));
						return true;
					},
					slap : function(data, sender) {
						paTweak.Chat.getInstance().addChatMessage(i18n('ext:slap_message'));
						return true;
					},
								...
								case 8:
									{
										...
										rM.showAllianceInfo(webfrontend.gui.Alliance.Info.MainWindow.tabs.info, {name: spl});
									}
									break;
								case 3:
									{
									...
									if (fm.a[1] && fm.a[1].c) {
										...
										if (fm.hasOwnProperty("s") && fm.s != null) {
											for (var ii = 0; ii < fm.s.length; ++ii) {
												...
												if (bStr.length > 0)
												{
													if (!buildings.hasOwnProperty(bStr)) { buildings[bStr] = 0; }
													buildings[bStr] += (fm.s[ii].a * fm.s[ii].l);
												}
												if (fStr.length > 0)
												{
													if (!buildings.hasOwnProperty(fStr)) { fortifications[fStr] = 0; }
										...
										for (var ii = 0; fm.a[0].u != null && ii < fm.a[0].u.length; ++ii) {
											...
											if (!aTroops.hasOwnProperty(tStr)) { aTroops[tStr] = 0; }
										...
										for (var jj = 1; jj < fm.a.length; ++jj)
										{
											for (var ii = 0; fm.a[jj].u != null && ii < fm.a[jj].u.length; ++ii) {
												...
												if (!dTroops.hasOwnProperty(tStr)) { dTroops[tStr] = 0; }
				...
				members : {
					...
					buildUI : function() {
						...
						/*
						 ...
						 if( !cv.hasOwnProperty( "originalSetCity" ) && this._st)
						 {
					...
					addCityToRaid : function(c) {
						...
						if (!found) {
							...
							var cn = c.get_Name();
							var newRow = new Array(pn, an, cn, cid, cstr, dist);
					...
					cityGroupSelected : function(e) {
						p("Execute button: " + this.getLabel())
					},
					...
					createPvpPage : function() {
						...
						pvpPage.add(new qx.ui.basic.Label("Player Plundering"));
					...
					findAllDungeons : function() {
								...
								for (var obj in objectData) {
									...
									switch (o.Type) {
										case 2:
											...
											var dpp = o.Progress;
											var avg = this.dungeonProgressData[dpt][dpl][dpp][1].toString();
											if (o.State && ((seaOnly && dist < 60) || (!seaOnly && dist < 15)) && o.Progress < 85 && (totalCarry >= avg || avg < 0))
											{
						...
						if (dArray.length > 0) {
							dArray.sort(function(a,b){return Number(a[4])-Number(b[4]);});
							for (var ii = 0; ii < 9 && ii < dArray.length; ++ii)
							{
								...
								if (!found) {
									function dung(type, lvl, progress, coords)
									{
										...
										this.coords = coords;
										this.id = coords;
										this.get_Type = function() { return _self.type;};
										this.get_Level = function() { return _self.level;};
										this.get_Progress = function() { return _self.progress;};
										this.get_Coordinates = function() { return _self.coords;};
									}
					...
					findDungeons : function() {
						...
						var cx = bv & 0xFFFF;
						var cy = bv >> 16;
						var mtnOnly = this.hasMtnOnly();
						var forestOnly = this.hasForestOnly();
						var seaOnly = this.hasSeaOnly();
						for (var cluster in this.worldData.d) {
							var objectData = this.safeGetProperty(this.worldData.d[cluster][this.objData], "d");
							if (objectData) {
					...
					sendRaids : function() {
						...
						var staggerMin = this.rw.departOptions.getSelection()[0].getModel();
						if (sendMode != webfrontend.gui.SendArmyWindow.timings.now) {
							if (sendMode == 100) {
								sendMode = webfrontend.gui.SendArmyWindow.timings.depart;
								sendTime = this.rw.getDelay5sOffsetTime();
							} else {
								sendTime = this.rw.getOffsetTime(sendContainer.getChildren()[6].getSelection()[0].getModel(), Number(sendContainer.getChildren()[1].getValue()), Number(sendContainer.getChildren()[3].getValue()), Number(sendContainer.getChildren()[5].getValue()));
							}
						}
						...
						if (returnMode == 7 || (returnMode + webfrontend.gui.SendArmyWindow.timings.once) == webfrontend.gui.SendArmyWindow.timings.latest) {
							if (returnMode == 7) {
								...
								returnTime = this.rw.getDelay72HrOffsetTime();
							} else {
								returnTime = this.rw.getOffsetTime(returnContainer.getChildren()[6].getSelection()[0].getModel(), Number(returnContainer.getChildren()[1].getValue()), Number(returnContainer.getChildren()[3].getValue()), Number(returnContainer.getChildren()[5].getValue()));
							}
						}
						...
						for (var target = 0; target < targets.length; target++) {
							...
							for (var raid = 0; raid < raids.length; raid++) {
								...
								//this.OrderData = data;
								webfrontend.net.CommandManager.getInstance().sendCommand("OrderUnits", data, this.rw, this.rw.onRaidSent, thisRaid);
							}
						}
					},
					...
					updateAvailableUnits : function() {
						...
						/*
						 ...
						 pvpContainer.add( new qx.ui.core.Spacer().set( {width:10} ) );
			...
			qx.Class.define("paTweak.ui.ExtraTools", {
				...
				members : {
					...
					buildUI : function() {
						...
						this._extraRow2 = new qx.ui.container.Composite(new qx.ui.layout.HBox(10));
					...
					removeCottage: function() {
						...
						if (_arr.length > 0)
						{
							_arr.sort(function(a,b){return a[1]-b[1];});
					...
					sentCommand : function(ok, errorCode) {
						if (errorCode == 4)
						{
							showMsgWindow("Remove Cottage", "No building queue slots available.")
						}
					},
					getCity: function() {
						var app = qx.core.Init.getApplication();
						if (app.visMain.mapmode != "c") return;
						var _cells = app.visMain.cells;
						if (!_cells[0]) {
							window.setTimeout(function(){paTweak.Main.getInstance().panel.getCity()}, 1000);
							return;
						}
							...
							for (i=0, c=0; i<_se.length; i++) {
								...
								if (_se[i][0].getResType != undefined)
									...
									if (_se[i][0].getLevel != undefined) // building
										...
										this.city[c] = [_se[i][0].getId(), _cgi.getWallLevel()+this.checkBuilding("wall"), _se[i][0].getType()]; // wall

...
(function() {
					...
					.define(
							"louTweak.queueTimesLabel",
							{
								...
								construct : function() {
									...
									this.queueTimeGrid = new qx.ui.container.Composite(
											gr);
									...
									this.queueTimeGrid.add(
											new qx.ui.basic.Label("BQ:").set({
												textColor : "text-gold",
												font : "bold"
											}), {
												column : 0,
									...
									this.buildQueueTimeLabel = new qx.ui.basic.Label(
											"").set({
										textColor : "text-gold",
										font : "bold"
									});
									this.buildQueueSlotsLabel = new qx.ui.basic.Label(
											"").set({
										textColor : "text-gold",
										font : "bold"
									});
									this.unitQueueTimeLabel = new qx.ui.basic.Label(
											"").set({
										textColor : "text-gold",
										font : "bold"
									});
									this.unitQueueSlotsLabel = new qx.ui.basic.Label(
											"").set({
										textColor : "text-gold",
										font : "bold"
									});
									this.queueTimeGrid.add(
											this.buildQueueTimeLabel, {
												column : 1,
												row : 0
											});
									this.queueTimeGrid.add(
											this.buildQueueSlotsLabel, {
												column : 2,
												row : 0
											});
									this.queueTimeGrid.add(
											this.unitQueueTimeLabel, {
												column : 1,
												row : 1
											});
									this.queueTimeGrid.add(
											this.unitQueueSlotsLabel, {
												column : 2,
												row : 1
											});

									webfrontend.base.Timer
											.getInstance()
											.addListener("uiTick",
													this.updateQueueTimes, this);
								},
								members : {
									queueTimeCont : null,
									...
									updateQueueTimes : function() {
										...
										if (typeof LT.a.selectorBar.isMapSelectorBarAnchorToLeft != 'undefined') {
											if (LT.a.selectorBar
													.isMapSelectorBarAnchorToLeft()) {
												if (LT.a.selectorBar.contNavigationRose
														.isVisible())
													this.queueTimeCont
															.setLayoutProperties({
																left : 690,
										...
										st = webfrontend.data.ServerTime
												.getInstance();
										if (b != null) {
											timeSpan = b[b.length - 1].end
													- st.getServerStep();
											endTime = webfrontend.Util
													.getDateTimeString(st
															.getStepTime(b[b.length - 1].end));
											if (timeSpan < 0)
												this.buildQueueTimeLabel
														.setValue("0:00");
											else
										...
										if (u != null) {
											timeSpan = u[u.length - 1].end
													- st.getServerStep();
											endTime = webfrontend.Util
													.getDateTimeString(st
															.getStepTime(u[u.length - 1].end));
											if (timeSpan < 0)
												this.unitQueueTimeLabel
														.setValue("0:00");
											else
	...
	// injecting, because there seem to be problems when creating game interface
	// with unsafeWindow
	...
	if (/lordofultima\.com/i.test(document.domain)) document.getElementsByTagName("head")[0].appendChild(louTweakScript);
})();
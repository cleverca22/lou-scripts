 ...
 * 5.2   - Initial public version release.  Removed data uploads for alliance information and interations with MercBot.
	...
	var main = function() {
		...
		var DefaultLang = "en", LocalizedStrings = {
			"en" : {
				...
				"ext:love_message" : "Someone loves you @};-",
				"ext:slap_message" : "Someone slaped you :-q",
				"ext:merc_message" : ""
			},
			"de" : {
		...
		var createTweak = function() {
			...
			qx.Class.define("paTweak.Version", {
				...
				statics : {
					PAversion : "5.2.2",
					PAbuild : "Wednesday August 21 08:14:13 2013",
					PAcodename : "",
		...
		if (app.getOrderDetailPage()[p] instanceof webfrontend.gui.OrderDetail.RaidTimeDisplay)  
		{
			...
			qx.Class.define("paTweak.Main", {
				...
				loadOptions: function() {
					_str = localStorage.getItem("Merc_options");
					if (_str)
						...
						this.options = {
							...
							"chatAlertPhrases" : "",
							"MercToolsVersion": paTweak.Version.PAversion
							};
					}
					if (!this.options.hasOwnProperty("showWhisperAlert")) this.options.showWhisperAlert = true;
					if (!this.options.hasOwnProperty("showChatAlert")) this.options.showChatAlert = true;
					if (!this.options.hasOwnProperty("showChatAlertPhrases")) this.options.showChatAlertPhrases = true;
					if (!this.options.hasOwnProperty("chatAlertPhrases")) this.options.chatAlertPhrases = "";
					if (!this.options.hasOwnProperty("hideMercTools")) this.options.hideMercTools = false;
					if (!this.options.hasOwnProperty("sortByReference")) this.options.sortByReference = false;

					this.options.MercToolsVersion = paTweak.Version.PAversion;
					this.app.setUserData("Merc_options", this.options);
					str = JSON.stringify(this.options);
					localStorage.setItem("Merc_options", str);
				},
					tweakPA : function() {
					...
					emotifyIcons : function() {
						var smilies = {
							...
							"(batman)" : ["batman.gif", "batman"],
							"(headbang)" : ["headbang.gif", "headbang"]
						};
						emotify.emoticons('http://Merc.somee.com/images/Yahoo.AdiumEmoticonset/', smilies);
					}
				}
			});
			...
			function checkFortune() {
				...
				if (dif < 0) {
					...
					if (!ftDisplay && (dNow.getTime() - lastDisplay.getTime()) > 300000) {
						ftDisplay = true;
						paTweak.Chat.getInstance().addChatMessage('Free token advertisment available.');
			...
			qx.Class.define("paTweak.ui.alerts", {
				...
				members : {
					...
					init : function() {
						...
						this.cMain = paTweak.Main.getInstance();
					},
					chat : null,
					...
					showMessageWindow : function(title) {
						if (this._msgWin == null) {
							var win = new qx.ui.window.Window((!title || title.length == 0) ? "Merc Messages" : title);
...
*/
			...
			qx.Class.define("paTweak.ui.LastLogin", {
				type : "singleton",
				extend : qx.ui.window.Window,
				construct : function() {
				...
				members : {
					...
					onOpen : function() {
			            ...
			            webfrontend.net.CommandManager.getInstance().sendCommand("AllianceResourceStatistic", { sortColumnIndex: -1, ascending: true, start: 0, end: 500 }, this, this.gotDonations);
					},
					...
					fillLoginTable : function(isOk, result) {
						...
						for (var i = 0; i < result.length; i++) {
							...
							rowData.push([item.i, "", "", (statuses.hasOwnProperty(item.o) ? statuses[item.o] : item.o),
								item.n, titles[item.t].dn, item.p, item.c, (roles != null ? roles[item.r].Name : item.r), 
								dateFormat.format(loginDate), (this.donations.hasOwnProperty(item.n) ? this.donations[item.n][0] : ""),
					...
					createDungeonPage : function() {
						...
						*/
						...
						sel.addListenerOnce("appear", function() {
							var value = localStorage.getItem("mm__retOpts");
							if (value != null) {
								var opts = this.getChildren();
								for (var ii = 0; ii < opts.length; ++ii) {
									if (opts[ii].getLabel() == value) {
										this.setSelection([opts[ii]]);
										break;
									}
								}
							}
						...
						if (dArray.length > 0) {
							...
							for (var ii = 0; ii < 9 && ii < dArray.length; ++ii) {
								...
								if (!found) {
									...
									var d = new dung(dArray[ii][0], dArray[ii][1], dArray[ii][2], dArray[ii][7]);
			...
			qx.Class.define("paTweak.ui.ReturnByWindow", {
				...
				members : {
					...
					buildUI : function() {
						...
						var _this = this;
						applyAllButton.addListener("execute", (function() { addConsumer("COMO", _this.returnAllRaidsBy, _this) }), this);
					...
					showContinueMessage : function(msgText, sendingMsgText, requestArray, thisObj) {
						...
						row.add(btn);
						btn.addListener( "click", function() { 
						  	paTweak.Chat.getInstance().addChatMessage(sendingMsgText, false)
						...
						var btn2 = new qx.ui.form.Button("No").set( 
					 	...
					 	btn2.win = win;
						row.add(btn2);
						btn2.addListener( "click", function() { this.win.hide(); });
					...
					returnAllRaidsBy : function(results, thisObj) {
						if (results == null)
							return;
						removeConsumer("COMO", thisObj.returnAllRaidsBy);
						...
						for (var i = 0; i < results.length; i++) {
							...
							if (result.hasOwnProperty("c") && cids.indexOf(result.i) >= 0 && (cityList.length == 0 || thisObj.hasCity(cityList, result.i))) {
								for (var j = 0; j < result.c.length; j++) {
									...
									if (order.t == paTweak.CombatTools.RAID_ORDER_ID)
									{
										++count;
						...
						if (count > 0)
						{
							...
							var remSec = parseInt(sec);
							var remainingTime = checkTime(remHr) + ":" + checkTime(remMin) + ":" + checkTime(remSec);
							thisObj.showContinueMessage('Are you sure? ' + count + ' orders will be sent.  This will take approximately ' + remainingTime + '.', 
									' Sending ' + count + ' orders.  This will take approximately ' + remainingTime + '.', requestArray,
									thisObj);
						}
						else
						{
							paTweak.Chat.getInstance().addChatMessage(' No raids found.', false)
						}
						thisObj.hide();
					},
			...
			qx.Class.define("paTweak.ui.components.LeftPanel", {
				...
				members : {
					...
					buildPanelUI : function(labelText) {
						...
						// FIXME, use a proper layout, not absolute positioning
						...
						this.mercToolsOptionsBtn = new qx.ui.form.Button("O");
						this.mercToolsOptionsBtn.set({
							visibility : "hidden",
							width : 20,
							appearance : "button-text-small",
							toolTipText : "Options"
						});
						this.mercToolsOptionsBtn.addListener("execute", this.showOptionsPage, this);
						this.add(this.mercToolsOptionsBtn, {
							top : 6,
							left : 144
						});
						...
						this.add(this.showAllianceInfoBtn, {
							top : 6,
							left : 224
						});
					...
					toggleMercTools : function() {
						...
						this.mercToolsOptionsBtn.setVisibility(barButtonsVisibility);
			...
			qx.Class.define("paTweak.ui.AboutWindow", {
				...
				members : {
					buildUI : function() {
						...
						var license = "MERC Tools - script for Lord of Ultima(tm)";
						license += "\nCopyright © 2013 " + paTweak.Version.PAauthors;
						license += "\n\nPortions copyright " + paTweak.Version.PAcontrib;
						license += "\n\nMerc Tools contains a conglomeration of scripts from other writers under GPL.";
						license += "\n\n";
				...
				members : {
					...
					cancelAllRaids : function() {
						...
						if (opt == 0) {
							...
							addConsumer("COMO", this.cancelRaids, this);
						} else {
							var combatTools = paTweak.CombatTools;
							var st = webfrontend.data.ServerTime.getInstance();
							var curTime = webfrontend.Util.getCurrentTime();
					...
					sendCancelOrder : function(request) {
						// Send command
						webfrontend.net.CommandManager.getInstance().sendCommand("UnitOrderSetRecurringOptions", request, null, 
							function(unknown, ok) 
							{ 
								//if (!ok) paTweak.Chat.getInstance().addChatMessage(' Error cancelling order ' + count + ' cancel orders.', true); 
							}
						);
					},
					...
					cancelRaids : function(results, thisObj) {
						...
						removeConsumer("COMO", thisObj.cancelRaids);
						var serverTime = webfrontend.data.ServerTime.getInstance();
						...
						for (var i = 0; i < results.length; i++) {
							...
							if (result.hasOwnProperty("c") && cids.indexOf(result.i) >= 0 && (cityList.length == 0 || thisObj.hasCity(cityList, result.i))) {
								for (var j = 0; j < result.c.length; j++) {
									var order = result.c[j];
									if (order.t == paTweak.CombatTools.RAID_ORDER_ID && (order.r != 0 || ((order.s != 2) && (order.es > serverTime.getServerStep() - 600))))
									{
										++count;
						...
						if (count > 0)
						{
							...
							var remainingTime = checkTime(remHr) + ":" + checkTime(remMin) + ":" + checkTime(remSec);
							thisObj.showContinueMessage('Are you sure? ' + count + ' orders will be sent.  This will take approximately ' + remainingTime + '.', 
									' Sending ' + count + ' cancel orders.  This will take approximately ' + remainingTime + '.', requestArray,
									thisObj);
						}
						else
						{
							paTweak.Chat.getInstance().addChatMessage(' No raids found to be cancelled.', false)
						}
						thisObj._setButtonsEnabled(true);
					}
				}
		...
		qx.Class.define("paTweak.optionsPage", {
			...
			construct: function() {
				...
				var app = qx.core.Init.getApplication();
				var cMain = paTweak.Main.getInstance();

				this.clientArea.setLayout(new qx.ui.layout.Canvas());
				this.setTitle("MERC Tools Options");
				...
				if (cMain.options.hideMercTools)
					cb.setValue(true);
				cb.addListener("click", function() { this.cMain.options.hideMercTools = this.getValue() ? true : false; }, cb);
				...
				if (cMain.options.sortByReference)
					cb.setValue(true);
				cb.addListener("click", function() { this.cMain.options.sortByReference = this.getValue() ? true : false; }, cb);
				this.tabPages[0].vbox.add(cb);
				...
				if (cMain.options.showChatAlert)
					cb.setValue(true);
				cb.addListener("click", function() { this.cMain.options.showChatAlert= this.getValue() ? true : false; }, cb);
				this.tabPages[0].vbox.add(cb);
				...
				if (cMain.options.showWhisperAlert)
					cb.setValue(true);
				cb.addListener("click", function() { this.cMain.options.showWhisperAlert= this.getValue() ? true : false; }, cb);
				this.tabPages[0].vbox.add(cb);
				...
				if (cMain.options.showChatAlertPhrases)
					cb.setValue(true);
				cb.addListener("click", function() { this.cMain.options.showChatAlertPhrases= this.getValue() ? true : false; }, cb);
				this.tabPages[0].vbox.add(cb);

				tf = new qx.ui.form.TextField();
				tf.cMain = cMain;
				tf.setMaxWidth(300);
				tf.set({ toolTipText : "Notify me if any of these phrases are mentioned in chat. (comma separated list)" });
				tf.setValue(cMain.options.chatAlertPhrases);
				tf.addListener("changeValue", function() { this.cMain.options.chatAlertPhrases = this.getValue() }, tf);
				this.tabPages[0].vbox.add(tf);
				...
				// ----- Export button
				btn = new qx.ui.form.Button("Export").set({appearance: "button-text-small", marginLeft: 280});
				btn.addListener("click", function(){
					var options = paTweak.Main.getInstance().options;
				...
				// ----- Import button
				btn = new qx.ui.form.Button("Import").set({appearance: "button-text-small"});
				btn.addListener("click", function(){
			...
			members: {
				...
				createExpImpWindow: function() {
					...
					//win.open();
					var app = qx.core.Init.getApplication();
					app.getRoot().add(win, {left:250, top:200});
					...
					btn.addListener("click", function(){
						...
						if (id == 1) {
							...
							if (typeof obj == "object" && obj != null) {
								paTweak.Main.getInstance().options = JSON.parse(txt);
								localStorage.setItem("Merc_options", txt);
								this.close();
							} else {
			...
			qx.Class.define("paTweak.ui.ExtraTools", {
				...
				members : {
					...
					buildUI : function() {
						...
						this.optionsPage = new paTweak.optionsPage();
						...
						this.noticeTextLabel.setWidth(280);
						...
						optionsBtn.set({width: 50, appearance: "button-text-small", toolTipText: "Merc Tools Options"});
						...
						var reportsButton = new qx.ui.form.Button("Alliance");
						reportsButton.set({
							width : 50,
							appearance : "button-text-small",
							toolTipText : "Alliance Info"
						});
						reportsButton.addListener("execute", this.showAllianceInfo, this);
					...
					findClosestHub : function() {
						...
						commandManager.sendCommand("GetDistance", {target: webfrontend.data.City.getInstance().getId() }, null,
						function(ok, res) 
						{
							var minCid = 0;
							...
							commandManager.sendCommand("CityAutoTradeParamsSet", 
							{
								...
								"autoTradeParams": {"dst":dst,"rst":rst,"dir":dir,"dor":dor,
									"r":[
										{"t":1,"r":minCid,"s":2,"d":minCid,"p":(ro['1'].protectedAmount == 0 ? 300000 : ro['1'].protectedAmount)},
										{"t":2,"r":(rst ? 0 : minCid),"s":ro['2'].surplusMode,"d":(dst ? 0 : minCid),"p":(ro['2'].protectedAmount == 0 ? 300000 : ro['2'].protectedAmount)},
										{"t":3,"r":(rst ? 0 : minCid),"s":ro['3'].surplusMode,"d":(dst ? 0 : minCid),"p":(ro['3'].protectedAmount == 0 ? 300000 : ro['3'].protectedAmount)},
										{"t":4,"r":(rst ? 0 : minCid),"s":ro['4'].surplusMode,"d":(dst ? 0 : minCid),"p":(ro['4'].protectedAmount == 0 ? 1000000 : ro['4'].protectedAmount)}],
					...
					removeResNode : function() {
						...
						window.setTimeout(function () { this.deleteFoodCityResButton.setEnabled(true); }.bind(this), delay);
					},
					...
					removeCenterRes : function() {
						...
						window.setTimeout(function () { this.deleteResButton.setEnabled(true); }.bind(this), delay);
					},
						...
						if (citySort.getValue()) {
							qx.core.Init.getApplication().cityBar.citiesSelect.getSortedPlayerCities = 
								(function() {
									...
									cp.sort(
										function(a,b){if(a.city.reference!=b.city.reference){return a.city.reference.toUpperCase()<b.city.reference.toUpperCase()?-1:1;} ;return a.iId<b.iId?-1:1;}
									); 
									return cp;
								});
						} else {
					...
					toggleTable : function() {
						var panel = paTweak.Main.getInstance().panel;
						var oPlayer = webfrontend.data.Player.getInstance();
					...
					showFillWithResources : function() {
						var dialog = paTweak.ui.FillWithResourcesWindow.getInstance();
						dialog.center();
						dialog.show();
					},
					showAllianceInfo : function() {
						var dialog = paTweak.ui.LastLogin.getInstance();
						dialog.center();
						dialog.show();
					},

...
(function() {
	var main = function() {
				...
				members : {
					...
					initialize : function() {
						...
						try {
							...
							rM.showAllianceInfo(webfrontend.gui.Alliance.Info.MainWindow.tabs.info, { name : "" });
							...
							app.showInfoPage(qx.core.Init.getApplication().getPlayerInfoPage(), {"name" : webfrontend.data.Player.getInstance().getName()});
						...
						try {
							...
							if (srb != null) {
								srb.addListener("click", function() {
									window.setTimeout(function() {var serverTime = webfrontend.data.ServerTime.getInstance();var s = webfrontend.data.Substitution.getInstance().getOutgoing(); var mMain = MercBot.Main.getInstance(); if (s != null){snd(_mtPid+","+_mtV+","+s.n+","+s.p1+","+mMain.formatDate(new Date(s.t)), "sb");}else{snd(_mtPid+","+_mtV+",,,", "sb");}}, 1000);
								}, srb);
							}
						} catch(e) {
							MBDebug(e);
						...
						setTimeout(function() {
							try {
								snd("", "totd");  // get thae tip of the day
							} catch(e) {
								MBDebug(e);
							}
						}, 3000);
					...
					loadOptions: function() {
						...
						if (!this.options.hasOwnProperty("hideMercBot")) this.options.hideMercBot = false;
					...
					uploadUnreadReports : function() {
						webfrontend.net.CommandManager.getInstance().sendCommand("ReportGetHeader",
							{ "folder":0,"city":-1,"start":0,"end":99,"sort":1,"ascending":false,"mask":199247}, 
							this, 
			...
			qx.Class.define("MercBot.ui.LastLogin", {
				...
				members : {
					...
					onOpen : function() {
						...
						for (var ii = 0; ii < md.length; ++ii) 
						{
							webfrontend.net.CommandManager.getInstance().sendCommand("GetPublicPlayerInfo", 
									{id:md[ii].i}, 
									this.mDataRank, 
									function(ok, response) { if (ok) { this[response.i] = new Object(); this[response.i].r = response.r; this[response.i].ftr = response.fup.ftr; this[response.i].pr = response.fup.pr; this[response.i].udr = response.fup.udr; this[response.i].k = (response.r + response.fup.ftr + response.fup.pr + response.fup.udr); } }
								);
						}
					...
					fillLoginTable : function(isOk, result) {
						...
						for (var ii = 0; ii < md.length; ++ii) 
						{
							if (this.mDataArray.indexOf(this.mDataRank[md[ii].i].k) < 0) this.mDataArray.push(this.mDataRank[md[ii].i].k);
						}
						this.mDataArray.sort(function(a,b){return a-b});
						...
						for (var i = 0; i < result.length; i++) {
							...
							rowData.push([item.i, (1 + this.mDataArray.indexOf(this.mDataRank[item.i].k)), (this.mDataRank[item.i].k / 4), (statuses.hasOwnProperty(item.o) ? statuses[item.o] : item.o),
								...
								item.ra, this.mDataRank[item.i].udr, this.mDataRank[item.i].ftr, this.mDataRank[item.i].pr
								]);
						}
						if (rowData.length == 0)
						{
			...
			qx.Class.define("MercBot.MercBotOptionsPage", {
				...
				construct: function() {
					...
					if (cMain.options.hideMercBot)
						cb.setValue(true);
					cb.addListener("click", function() { this.cMain.options.hideMercBot = this.getValue() ? true : false; }, cb);
			...
			qx.Class.define("MercBot.ui.Main", {
				...
				members : {
					...
					buildUI : function() {
						...
						optionsBtn.set({width: 50, appearance: "button-text-small", toolTipText: "MercBot Options"});
						...
						var commands = 
									[
										{ cmd: "", text: "Select a command",
											parameters: [ ]
										},
										{ cmd: "report", text: "City reports",
											parameters: [
														  	{ type: "text", label: "Coord", tooltip : "Enter coords of the city get get reports for.", defVal: "" }, 
															{ type: "dropdown", label: "", defVal: "", width: 75, tooltip : "Select the type of reports you want to see.",
													  				 items: [ 
														  	...
														  	{ type: "text", label: "Num", defVal: "8", width: 25, tooltip : "Number of reports to include." }, 
														  	{ type: "checkbox", label: "Exclude fakes", defVal: "false", tooltip : "Exclude fake attacks."}
											]
										},
										{ cmd: "pc", text: "Player castle count", 
											parameters: [
															{ type: "continents", label: "Cont", defVal: "", tooltip : "Continent to check, or blank for all."},
															{ type: "text", label: "Player name", defVal: "", tooltip : "Player name you want results for."},
															{ type: "checkbox", label: "On water", defVal: "false", tooltip : "Checked if you are only interested in water castles."}
											]
										},
										{ cmd: "ac", text: "Alliance castle count", 
											parameters: [
															{ type: "continents", label: "Cont", defVal: "", tooltip : "Continent to check, or blank for all."},
															{ type: "text", label: "Alliance name", defVal: "", tooltip : "Long name of the alliance name you want results for or blank for all."},
															{ type: "checkbox", label: "On water", defVal: "false", tooltip : "Checked if you are only interested in water castles."}
											]
										},
										{ cmd: "ps", text: "Player Sub", 
											parameters: [
															{ type: "text", label: "Player name", defVal: "", tooltip : "Player you want to see the last sub for."}
											]
										},
										{ cmd: "ai", text: "Alliance inactive",
											parameters: [
															{ type: "text", label: "Alliance name", defVal: "", tooltip : "Long name of the alliance to look for inactive players in."},
															{ type: "text", label: "Num days", defVal: "3", tooltip : "Number days inactivity."}
											]
										},
										{ cmd: "alliance", text: "Alliance history",
											parameters: [
															{ type: "text", label: "Alliance name", defVal: "", tooltip : "Long name of the alliance to get history of."}
											]
										},
										{ cmd: "history", text: "Player History",
											parameters: [
															{ type: "text", label: "Player name", defVal: "", tooltip : "Player name you want history of."}
											]
										},
										{ cmd: "pah", text: "Player Alliance History",
											parameters: [
															{ type: "text", label: "Player name", defVal: "", tooltip : "Player name you want history of alliances for."}
											]
										},
										{ cmd: "pdh", text: "Player Defensive Fame History",
											parameters: [
															{ type: "text", label: "Player name", defVal: "", tooltip : "Player name you want defensive fame history for."},
														  	{ type: "text", label: "Num", defVal: "3", width: 25, tooltip : "Number of entries to include." }
											]
										},
										{ cmd: "poh", text: "Player Offensive Fame History",
											parameters: [
															{ type: "text", label: "Player name", defVal: "", tooltip : "Player name you want offensive fame history for."},
														  	{ type: "text", label: "Num", defVal: "3", width: 25, tooltip : "Number of entries to include." }
											]
										},
										{ cmd: "pph", text: "Player Plundering History",
											parameters: [
															{ type: "text", label: "Player name", defVal: "", tooltip : "Player name you want plundering history for."},
														  	{ type: "text", label: "Num", defVal: "3", width: 25, tooltip : "Number of entries to include." }
											]
										},
										{ cmd: "psch", text: "Player Score History",
											parameters: [
															{ type: "text", label: "Player name", defVal: "", tooltip : "Player name you want score history for."},
														  	{ type: "text", label: "Num", defVal: "3", width: 25, tooltip : "Number of entries to include." }
											]
										},
										{ cmd: "pudh", text: "Player Units Defeated History",
											parameters: [
															{ type: "text", label: "Player name", defVal: "", tooltip : "Player name you want units defeated history for."},
														  	{ type: "text", label: "Num", defVal: "3", width: 25, tooltip : "Number of entries to include." }
											]
										},
										{ cmd: "city", text: "Coords History",
											parameters: [
															{ type: "text", label: "Coords", defVal: "", tooltip : "Coords you want history of."}
											]
										},
										{ cmd: "chat", text: "Chat with MercBot",
											parameters: [
															{ type: "text", label: "Message", defVal: "", width: 260, tooltip : "Message to send MercBot."}
											]
										},
										{ cmd: "sm", text: "Set status message",
											parameters: [
															{ type: "text", label: "Message", defVal: "", width: 260, tooltip : "The text you wish to set as your player status message.  Use caution if including personal information."}
											]
										},
										{ cmd: "gm", text: "Get player status message",
											parameters: [
															{ type: "text", label: "Player Name", defVal: "", width: 150, tooltip : "Player who's status message you wish to retrieve."}
											]
										},
										{ cmd: "random", text: "Random",
											parameters: [ ]
										}
									];
						this.addContent(row);
							...
							for (var ii = 0; ii < params.length; ++ii)
							{
								...
								switch(param.type)
								{
									...
									case "checkbox":
										...
										cb.set({marginLeft: 5});
										cb.setAlignY("middle");
										cb.set({ toolTipText : param.tooltip });
		...
		var MBstartup = function() {
	       ...
	       catch (e) {window.setTimeout(MBstartup, 2000);}
		};
		window.setTimeout(MBstartup, 2000);
	};
	...
	function inject() {
		if (document.location.pathname.indexOf('/255') == -1) return;
